...
export default function AdopsiOrganisasiForm({
  ...
  onSubmit: (data: { name: string; npp: string; nominal: string; adoptionPeriod: string }) => void;
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<{ name: string; npp: string; nominal: string; adoptionPeriod: string }>({
    defaultValues: {
      name: "",
      npp: "",
      nominal: "",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-bold text-muted-foreground">Nama Organisasi/Perusahaan:</label>
        <Input
      ...
      <div>
        <label className="block text-sm font-medium text-muted-foreground">NPP</label>
        <Input
          {...register("npp", { validate: validateNPP })}
          placeholder="Masukkan NPP"
          className="border-primary focus:ring-primary"
        />
        {errors.npp && <p className="text-sm text-red-500">{errors.npp.message}</p>}
      </div>
