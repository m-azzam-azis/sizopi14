...
import { ReservationStatus, ReservasiTiket } from "@/types/schema";

const adminReservasiFormSchema = z.object({
...
type AdminReservasiFormValues = z.infer<typeof adminReservasiFormSchema>;

interface AdminReservasiFormProps {
  onSubmit: (data: AdminReservasiFormValues) => void;
  initialData: ReservasiTiket;
}

export const AdminReservasiForm = ({
  onSubmit,
  initialData,
}: AdminReservasiFormProps) => {
  const form = useForm<AdminReservasiFormValues>({
    resolver: zodResolver(adminReservasiFormSchema),
    defaultValues: {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          <FormItem>
            ...
            <Input value={initialData.jadwal} disabled className="bg-muted" />
          </FormItem>
        </div>
