...
export async function GET(request: Request) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to fetch medical records" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
...
export async function DELETE(request: Request) {
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to delete medical record" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
...
export async function PUT(request: Request) {
  try {
    ...
    if (data.status_kesehatan === "Sedang Sakit") {
      // Use the enhanced method to capture PostgreSQL notices
      result = await catatanMedisModel.updateWithNotices(
        "id_hewan",
        data.id_hewan,
        data
      );
      ...
      const updatedRecord = await catatanMedisModel.update(
        "id_hewan",
        data.id_hewan,
        data
      );
    ...
    return new Response(
      JSON.stringify({ error: "Failed to update medical record" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
